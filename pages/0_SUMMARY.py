...
while True:
    ...
    buy = pd.DataFrame(columns=['Stock','Down%', 'Down_LB%', 'CMP', 'LB','Amount', 'Qty'])
    if time.time() - st.session_state.last_analysis_time >= 0:
        ...
        for stock in stocks:
            ...
            if last_buy == 0 or down_lb >= 10:
                new_res = pd.DataFrame({'Stock': [stock], 'Down%':[round(pnl*100,2)], 'Down_LB%':[down_lb],'CMP':[cmp], 'Amount': [amount], 'Qty': [qty], 'LB': [last_buy]})
        ...
        buy_Stock.dataframe(buy.sort_values('Down_LB%'), use_container_width=True)
